...
2. **postinstall runs builds**: `pnpm i` triggers a `postinstall` hook that builds `@vx-oss/heroui-v3-styles` and runs `typegen:docs`. If it fails (e.g., network issue on first install), run `pnpm --filter @vx-oss/heroui-v3-styles build` manually.
...
6. **Build order matters**: Turbo handles `^build` dependencies. `@vx-oss/heroui-v3-styles` must build before `@vx-oss/heroui-v3-react`. Running `pnpm build` from root handles this automatically.
