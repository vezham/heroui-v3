...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm i @vx-oss/heroui-v3-styles@beta @vx-oss/heroui-v3-react@beta
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm add @vx-oss/heroui-v3-styles@beta @vx-oss/heroui-v3-react@beta
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @vx-oss/heroui-v3-styles@beta @vx-oss/heroui-v3-react@beta
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @vx-oss/heroui-v3-styles@beta @vx-oss/heroui-v3-react@beta
    ```
  </Tab>
</Tabs>
...
`Switch.Content` groups label and description next to the switch control ([#6240](https://github.com/vezham/heroui-v3/pull/6240)).

**Before:**
```tsx
import { Switch, Label, Description } from '@vx-oss/heroui-v3-react';
...
The [Tabs](/docs/components/tabs) component now includes an explicit `Tabs.Separator` sub-component for adding visual separator lines between tabs. This replaces the previous automatic CSS pseudo-element separator and the `hideSeparator` prop ([#6243](https://github.com/vezham/heroui-v3/pull/6243)).
...
Props passed to `Breadcrumbs.Item` are now forwarded to the underlying `Link` ([#6233](https://github.com/vezham/heroui-v3/pull/6233)).
...
The `hideSeparator` prop has been removed from the Tabs component. Separators are now **opt-in** using the new `<Tabs.Separator />` sub-component instead of being automatically generated via CSS pseudo-elements ([#6243](https://github.com/vezham/heroui-v3/pull/6243)).
...
`DateInputGroup` and `ColorInputGroup` are no longer exported directly from `@vx-oss/heroui-v3-react`. Their sub-components have been consolidated under their respective field components (`DateField`, `TimeField`, `ColorField`).
...
**After:**
```tsx
import {DateField, Label, Description} from '@vx-oss/heroui-v3-react';
...
```tsx
import {ColorField, Label, ColorInputGroup, ColorSwatch} from '@vx-oss/heroui-v3-react';
...
**After:**
```tsx
import {ColorField, Label, ColorSwatch} from '@vx-oss/heroui-v3-react';
...
- [GitHub PR #6237](https://github.com/vezham/heroui-v3/pull/6237)
