...
**Styles separated from implementation:** `@vx-oss/heroui-v3-styles` is standalone CSS. `@vx-oss/heroui-v3-react` handles behavior. Use the styles with React, plain HTML + Tailwind, or any framework. BEM class names make every slot customizable globally. Swap themes to change not just variables, but how components look and feel.

**Headless when you want it:** Remove the `@vx-oss/heroui-v3-styles` import and you have a headless library. We maintain functionality and accessibility. You focus on your product.
...
@import "@vx-oss/heroui-v3-styles/components/card.css" layer(components);
```
...
```tsx
// Root import
import { Button, Card, Table } from "@vx-oss/heroui-v3-react";

// Subpath import
import { Button } from "@vx-oss/heroui-v3-react/button";
import { Card } from "@vx-oss/heroui-v3-react/card";
import { Table } from "@vx-oss/heroui-v3-react/table";
```
...
```json
{
  "mcpServers": {
    "heroui-react": {
      "command": "npx",
      "args": ["-y", "@vx-oss/heroui-v3-react-mcp@latest"]
    }
  }
}
```
...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm i @vx-oss/heroui-v3-styles @vx-oss/heroui-v3-react
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
