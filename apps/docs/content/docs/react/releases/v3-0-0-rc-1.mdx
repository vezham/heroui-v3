...
<Tabs items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">
    ```bash
    npm i @vx-oss/heroui-v3-styles@rc @vx-oss/heroui-v3-react@rc
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm add @vx-oss/heroui-v3-styles@rc @vx-oss/heroui-v3-react@rc
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @vx-oss/heroui-v3-styles@rc @vx-oss/heroui-v3-react@rc
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @vx-oss/heroui-v3-styles@rc @vx-oss/heroui-v3-react@rc
    ```
  </Tab>
</Tabs>
...
Table and ListBox now support virtualization via React Aria's `Virtualizer` for large datasets. `Virtualizer`, `TableLayout`, and `ListLayout` are re-exported from `@vx-oss/heroui-v3-react`.
...
`@vx-oss/heroui-v3-react` now supports per-component subpath entrypoints for more explicit imports ([#6301](https://github.com/vezham/heroui-v3/pull/6301)):
...
import { Button } from "@vx-oss/heroui-v3-react/button";
```
...
- **InputGroup**: Focus styles now trigger only when the actual input/textarea is focused (`:has([data-slot]:focus)`) instead of any focusable child via `:focus-within` ([#6274](https://github.com/vezham/heroui-v3/pull/6274))
- **Avatar**: Fallback element inherits `border-radius` from the parent instead of hardcoding `rounded-full`, so `className` overrides apply correctly ([#6300](https://github.com/vezham/heroui-v3/pull/6300))
- **Modal & AlertDialog**: Backdrop click events no longer propagate through portals to parent elements ([#6297](https://github.com/vezham/heroui-v3/pull/6297))
- **Table**: Fixed header rounding and background color bleeding in Firefox ([#6298](https://github.com/vezham/heroui-v3/pull/6298))
...
- [GitHub PR #6285](https://github.com/vezham/heroui-v3/pull/6285)
