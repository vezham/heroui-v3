...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ```tsx
    import { Navbar, NavbarBrand, NavbarContent, NavbarItem, Link, Button } from "@vx-oss/heroui-v3-react";
  ...
  <Tab value="v3">
    ```tsx
    import { Link, Button } from "@vx-oss/heroui-v3-react";
...
2. **Import Change**: Remove all Navbar imports from `@vx-oss/heroui-v3-react`
