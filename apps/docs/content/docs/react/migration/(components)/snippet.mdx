...
**v2:** `<Snippet>` component from `@vx-oss/heroui-v3-react`  
...
```tsx
import { Snippet } from "@vx-oss/heroui-v3-react";

export default function App() {
  return (
    <Snippet symbol="$">
      npm install @vx-oss/heroui-v3-react
    </Snippet>
  );
}
```
...
export default function App() {
  ...
  const handleCopy = async () => {
    await navigator.clipboard.writeText("npm install @vx-oss/heroui-v3-react");
    setCopied(true);
...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ...
    <Snippet symbol="$">
      {[
        "npm install @vx-oss/heroui-v3-react",
        "yarn add @vx-oss/heroui-v3-react",
        "pnpm add @vx-oss/heroui-v3-react"
      ]}
    </Snippet>
    ```
  </Tab>
...
2. **Import Change**: Remove `import { Snippet } from "@vx-oss/heroui-v3-react"`
...
1. **Remove Import**: Remove `Snippet` from `@vx-oss/heroui-v3-react` imports
