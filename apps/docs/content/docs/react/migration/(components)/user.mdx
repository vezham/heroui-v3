...
**v2:** `<User>` component from `@vx-oss/heroui-v3-react`  
...
```tsx
import { User } from "@vx-oss/heroui-v3-react";
...
<Tabs items={["v2", "v3"]}>
  <Tab value="v2">
    ```tsx
    import { User, Link } from "@vx-oss/heroui-v3-react";
...
2. **Import Change**: Remove `import { User } from "@vx-oss/heroui-v3-react"`
...
1. **Remove Import**: Remove `User` from `@vx-oss/heroui-v3-react` imports
