...
- **Using `@vx-oss/heroui-v3-react`**: Use Strategy A (pnpm aliases)
- **Using component packages** (`@vx-oss/heroui-v3-button`, `@vx-oss/heroui-v3-card`, etc.): Use Strategy B (component packages)
...
{
  "dependencies": {
    "@vx-oss/heroui-v3-react": "2.8.6",
    "@vx-oss/heroui-v3-theme": "2.4.24",
    "@vx-oss/heroui-v3-react": "npm:@vx-oss/heroui-v3-react@latest",
    "@vx-oss/heroui-v3-styles": "npm:@vx-oss/heroui-v3-styles@latest"
  }
}
```

2. Import v2 components from `@vx-oss/heroui-v3-react`:

```tsx
import {Button} from "@vx-oss/heroui-v3-react"; // v2
```

3. Import v3 components from `@vx-oss/heroui-v3-react`:

```tsx
import {Button} from "@vx-oss/heroui-v3-react"; // v3
```

### Migration Process

1. **Migrate one component at a time:**
   - Update imports to use `@vx-oss/heroui-v3-react`
...
3. **Once all components are migrated, switch to v3-only:**
   - Remove v2 dependencies (`@vx-oss/heroui-v3-react`, `@vx-oss/heroui-v3-theme`)
   - Remove aliases
   - Update all imports to `@vx-oss/heroui-v3-react` (remove `-v3` suffix)
...
{
  "dependencies": {
    "@vx-oss/heroui-v3-react": "latest", // v3
    "@vx-oss/heroui-v3-styles": "latest", // v3
    "@vx-oss/heroui-v3-button": "2.8.6", // v2
    "@vx-oss/heroui-v3-card": "2.8.6", // v2
    // ... other v2 component packages as needed
  }
}
```

2. Import v3 components from `@vx-oss/heroui-v3-react`:
...
```tsx
import {Card} from "@vx-oss/heroui-v3-card"; // v2
```

### Migration Process

1. **Install v3 packages** (`@vx-oss/heroui-v3-react`, `@vx-oss/heroui-v3-styles`)
2. **Install v2 component packages** for components not yet migrated
3. **Migrate one component at a time:**
   - Remove the v2 component package from dependencies
   - Update imports to use `@vx-oss/heroui-v3-react` (v3)
   - Update component code to v3 API
   - Test the migrated component
   - For v2 components with no v3 counterpart (Code, Image, Navbar, Ripple, Snippet, Spacer, User), leave them in place. They will continue to work during coexistence.
...
<Callout type="warning">
  **Important:** Import order matters. Import `tailwindcss` first, then `@vx-oss/heroui-v3-styles`.
</Callout>
...
```js
const {heroui} = require("@vx-oss/heroui-v3-react");
...
2. **Update imports**
   - Strategy A: Change `@vx-oss/heroui-v3-react` → `@vx-oss/heroui-v3-react`
   - Strategy B: Change component package → `@vx-oss/heroui-v3-react`
...
2. **Guide conflict resolution**
   - Ensure CSS import order: `tailwindcss` first, then `@vx-oss/heroui-v3-styles`
...
1. **Remove v2 dependencies:**
   - Remove `@vx-oss/heroui-v3-react` and `@vx-oss/heroui-v3-theme` (Strategy A)
   - Remove all `@vx-oss/heroui-v3-*` component packages (Strategy B)
...
2. **Update all imports:**
   - Change `@vx-oss/heroui-v3-react` → `@vx-oss/heroui-v3-react` (Strategy A)
   - Change component package imports → `@vx-oss/heroui-v3-react` (Strategy B)

3. **Update CSS:**
   - Remove v2 Tailwind plugin from config
   - Keep only `@import "@vx-oss/heroui-v3-styles";`
