...
1. **Identify hook usage**: Search codebase for imports from `@vx-oss/heroui-v3-react` that include hook names (`useSwitch`, `useInput`, `useCheckbox`, `useRadio`, `useDisclosure`, etc.)
...
1. **Update dependencies**
   - Remove `@vx-oss/heroui-v3-react` and `@vx-oss/heroui-v3-theme` (v2)
   - Install `@vx-oss/heroui-v3-react` and `@vx-oss/heroui-v3-styles` (v3)
   - Remove `framer-motion` if present
   - Update CSS imports (add `@import "@vx-oss/heroui-v3-styles";`)
