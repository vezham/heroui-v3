...
1. **Identify the strategy**: Check if project uses pnpm aliases (Strategy A) or component packages (Strategy B)
   - **Strategy A**: Look for aliases like `"@vx-oss/heroui-v3-react": "npm:@vx-oss/heroui-v3-react@latest"` in package.json
   - **Strategy B**: Look for component-specific packages like `@vx-oss/heroui-v3-button`, `@vx-oss/heroui-v3-card` alongside `@vx-oss/heroui-v3-react`

2. **Verify setup**: Ensure the coexistence setup is correct:
   - Strategy A: Both `@vx-oss/heroui-v3-react` (v2) and `@vx-oss/heroui-v3-react` (v3 alias) are installed
   - Strategy B: `@vx-oss/heroui-v3-react` (v3) and component packages like `@vx-oss/heroui-v3-button` (v2) are installed
...
2. **Update imports**
   - Change imports from `@vx-oss/heroui-v3-react` to `@vx-oss/heroui-v3-react`
   - Example: `import {Button} from "@vx-oss/heroui-v3-react"` → `import {Button} from "@vx-oss/heroui-v3-react"`
...
2. **Remove component package**
   - Remove the v2 component package from dependencies (e.g., `@vx-oss/heroui-v3-button`)
   - Update package.json

3. **Update imports**
   - Change imports from component package to `@vx-oss/heroui-v3-react` (v3)
   - Example: `import {Card} from "@vx-oss/heroui-v3-card"` → `import {Card} from "@vx-oss/heroui-v3-react"`
...
1. **Remove v2 dependencies**
   - Strategy A: Remove `@vx-oss/heroui-v3-react`, `@vx-oss/heroui-v3-theme`, and aliases
   - Strategy B: Remove all remaining `@vx-oss/heroui-v3-*` component packages
...
2. **Update all imports**
   ...
   - Strategy B: All imports should already be `@vx-oss/heroui-v3-react` (v3)
...
- Projects using unified `@vx-oss/heroui-v3-react` package (Strategy A can work but full migration may be simpler)
