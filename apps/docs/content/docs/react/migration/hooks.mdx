...
1. **Identify hook usage**: Search your codebase for imports from `@vx-oss/heroui-v3-react` that include hook names (`useSwitch`, `useInput`, `useCheckbox`, `useRadio`, etc.)
...
```tsx
import { useSwitch } from "@vx-oss/heroui-v3-react";
...
**Example:**
```tsx
import { Modal, Button, useOverlayState } from "@vx-oss/heroui-v3-react";
