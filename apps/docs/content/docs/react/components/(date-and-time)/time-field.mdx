...
```tsx
import {TimeField, Label, Description, FieldError} from '@vx-oss/heroui-v3-react';
...
```tsx
import {TimeField, Label, Description} from '@vx-oss/heroui-v3-react';
...
- **TimeField.Input** - Input component with segmented editing from `@vx-oss/heroui-v3-react`
