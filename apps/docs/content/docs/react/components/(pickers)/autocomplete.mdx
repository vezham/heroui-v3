...
```tsx
import { Autocomplete, useFilter } from "@vx-oss/heroui-v3-react";
```
...
```tsx
import {Autocomplete, Label, Description, SearchField, ListBox} from "@vx-oss/heroui-v3-react";
...
```tsx
import {Autocomplete, SearchField, ListBox} from "@vx-oss/heroui-v3-react";
...
The Autocomplete component uses these CSS classes ([View source styles](https://github.com/vezham/heroui-v3/blob/timeline/packages/styles/components/autocomplete.css)):
