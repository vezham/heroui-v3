...
```tsx
import { Select } from "@vx-oss/heroui-v3-react";
```
...
```tsx
import {Select, Label, Description, Header, ListBox, Separator} from "@vx-oss/heroui-v3-react";
...
The Select component uses these CSS classes ([View source styles](https://github.com/vezham/heroui-v3/blob/timeline/packages/styles/components/select.css)):
