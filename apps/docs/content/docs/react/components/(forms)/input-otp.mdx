...
```tsx
import { InputOTP } from '@vx-oss/heroui-v3-react';
```
...
The InputOTP component uses these CSS classes ([View source styles](https://github.com/vezham/heroui-v3/blob/timeline/packages/styles/components/input-otp.css)):
...
```tsx
import { REGEXP_ONLY_DIGITS, REGEXP_ONLY_CHARS, REGEXP_ONLY_DIGITS_AND_CHARS } from '@vx-oss/heroui-v3-react';
