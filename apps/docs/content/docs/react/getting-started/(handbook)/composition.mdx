...
HeroUI's variant functions are available in the `@vx-oss/heroui-v3-styles` package, which can be used independently of React. This enables Vue, Svelte, and other frameworks to use HeroUI's design system:

```tsx
// Import directly from @vx-oss/heroui-v3-styles (framework-agnostic)
import { buttonVariants } from '@vx-oss/heroui-v3-styles';

// Or import from @vx-oss/heroui-v3-react (re-exports the same functions)
import { buttonVariants } from '@vx-oss/heroui-v3-react';
```

Both imports work identically. Use `@vx-oss/heroui-v3-styles` when building for non-React frameworks or when you want to avoid pulling in React dependencies.
...
For more control and type safety, use variant functions to apply HeroUI styles to framework-specific components or custom elements. Variant functions are available from both `@vx-oss/heroui-v3-styles` (framework-agnostic) and `@vx-oss/heroui-v3-react` (re-exports).
...
**Available variant functions:** Each component exports its variant function (`buttonVariants`, `chipVariants`, `linkVariants`, `spinnerVariants`, and more) from `@vx-oss/heroui-v3-styles`. Use them to apply HeroUI's design system to any element while maintaining type safety.
...
```tsx
import { Alert } from '@vx-oss/heroui-v3-react';
...
```tsx
import { Alert, AlertTitle, AlertDescription } from '@vx-oss/heroui-v3-react';
...
// Or named export
import { ButtonRoot } from '@vx-oss/heroui-v3-react';
...
```tsx
import { Link } from '@vx-oss/heroui-v3-react';
import { linkVariants, buttonVariants } from '@vx-oss/heroui-v3-styles';
...
```tsx
import type { ButtonRootProps } from "@vx-oss/heroui-v3-react";
import type { VariantProps } from "tailwind-variants";

import { Button } from "@vx-oss/heroui-v3-react";
import { buttonVariants, tv } from "@vx-oss/heroui-v3-styles";
...
```tsx
import type { ButtonRootProps, AvatarRootProps } from "@vx-oss/heroui-v3-react";
...
```tsx
import { Button, Avatar } from "@vx-oss/heroui-v3-react";
...
Since `@vx-oss/heroui-v3-styles` has no React dependencies, you can use it directly in any framework:
```vue
<script setup>
import { buttonVariants } from '@vx-oss/heroui-v3-styles';
