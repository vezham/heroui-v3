...
```tsx
import { Tabs, TabList, Tab, TabPanel } from '@vx-oss/heroui-v3-react';
...
import { Alert, AlertIcon, CardHeader, AccordionTrigger } from '@vx-oss/heroui-v3-react';
...
Styles (`@vx-oss/heroui-v3-styles`) are separate from logic (`@vx-oss/heroui-v3-react`), enabling use with any framework or vanilla HTML. See [Tailwind Play example](https://play.tailwindcss.com/vMYXzKPyUx).
